...
class Bot {
  ...
  attachListeners() {
    ...
    this.discord.on('message', (message) => {
      ...
      if (message.content.toLowerCase().startsWith('/names')) {
        const channelId = message.content.match(/<#(\d+)>/)?.[1] ?? message.channel.id;
        const ircChannel = this.channelMapping[channelId];
        if (this.channelUsers[ircChannel]) {
          const ircNames = this.channelUsers[ircChannel].values();
          const ircNamesArr = new Array(...ircNames);
          this.sendExactToDiscord(message.channel.id, 'Users in ' + ircChannel + "\n> " + ircNamesArr.join(', '));
        } else {
  ...
  findDiscordChannel(ircChannel) {
    const discordChannelName = ircChannel[0] == '#' ? this.invertedMapping[ircChannel.toLowerCase()] : ircChannel;
